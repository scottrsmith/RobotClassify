...
class prepData(object):
    def __init__(self, name, project, outFile=None):
        ...
        if project.modelType != tm.TRAIN_CLUSTERING:
            ...
            if df[project.targetVariable[name]].dtype == 'object':
                ...
                if project.targetVariableIsBoolean[name]:
                    df[project.targetVariable[name]] =\
                        (df[project.targetVariable[name]] ==
                         project.targetVariableTrueValue[name]).astype(int)
                    targetValue = 'Boolean'
                elif project.targetVariableConvertValues[name] is not None:
                    for fromValue, toValue in \
                            project.targetVariableConvertValues[name]:
                        df[project.targetVariable[name]].replace(
                            fromValue, toValue, inplace=True)
                    df[project.targetVariable[name]
                       ] = df[project.targetVariable[name]].astype(int)
                    targetValue = 'Integer'
                mlUtility.runLog('Converting target Variable {} for {}'.format(
                    project.targetVariable[name], targetValue))

            # Create new dataframe with dummy features
            col = [x for x in df.dtypes[(df.dtypes == 'object')].index] + [
                x for x in df.dtypes[(df.dtypes == 'category')].index]
            ...
            if outFile is not None:
                ...
                mlUtility.runLog(
                    'Writing prepped training File {}'.format(outFile))
        ...
        # mlUtility. traceLog(("project.testSize,
        #   random_state=project.randomState = ",
        #   project.testSize, project.randomState)
        if project.modelType == tm.TRAIN_CLASSIFICATION:
            ...
            self.X_train, self.X_test, self.y_train, self.y_test =\
                train_test_split(self.X, self.y, test_size=project.testSize,
                                 random_state=project.randomState)
    ...
    def removeTestingFileInX(self, project, targetVariable):
        ...
        if project.mergedTrainingAndTest:
            if project.mergedTrainingAndTestFileName in \
                    project.preppedTablesDF:
                testingMask = [flip(x) for x in self.X[project.isTrainingSet]]
                # testingMaskt = (self.X[project.isTrainingSet] == False)
                self.X.drop(project.isTrainingSet, axis=1, inplace=True)

                project.preppedTablesDF[project.mergedTrainingAndTestFileName]\
                    = self.X.loc[self.X[testingMask].index]

                if targetVariable is not None:
                    project.preppedTablesDF[
                        project.
                        mergedTrainingAndTestFileName].drop(
                        targetVariable, axis=1, inplace=True)
