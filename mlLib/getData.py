...
class getData (object):
    ...
    def __init__(self, name, type=None, description=None,
                 location=None, fileName=None, range=None,
                 sheetName=None, df=None, hasHeaders=False):
    ...
    def openTable(self):
        ...
        if self.type == 'csv':
            ...
            self.dataFrame = pd.read_csv(path, header=headerValue(self),
                                         low_memory=False)
# Comment Out Google APIs for easy Distribution
...
#            self.fixNan()
            ...
            path = self.validateFilePath()
            self.dataFrame = pd.read_excel(path, header=headerValue(self),
    ...
    def fixHeaders(self):
        ...
        self.dataFrame = self.dataFrame.drop([0], axis=0).rename(
            columns=newcolumns, index=newindex)
