...
# from http://eurekastatistics.com/
...
#    elif hasattr(series, 'datetime'):
        ...
        stats['1%'], stats['2.5%'], stats['5%'] = q[0.01], q[0.025], q[0.05],
        stats['95%'], stats['97.5%'], stats['99%'] = q[0.95], q[0.975], q[0.99]
...
def runExploreTest(function, project, explore, df, dfCopy, colName, stats):
    rules, dropped, cleanFunctions = function(
        project, explore, df, dfCopy, colName, stats)
...
def testTextSignificance(project, explore, df, dfCopy, name, stats):
    ...
    top = []
    sig = .01
    for topVals, c in sorted(d.items(),
    ...
    # project.addManualRuleForDefault(ed.CLEANDATA_REBUCKET_BY_INCLUDE,
    #                               'Name',  rebucket)
    explore.addExploreRules(
        rules, name, CLEANDATA_REBUCKET_BY_INCLUDE, value=top, key=None)
    cleanFunction.append(explore.getRuleFunction(
        CLEANDATA_REBUCKET_BY_INCLUDE, name, value=top,
        df=df, forPredict=True))

    # project.addManualRuleForDefault(ed.CLEANDATA_SET_CATEGORY_DATATYPE,
    #                               'Name',None)
    explore.addExploreRules(rules, name, CLEANDATA_REFACTOR_DATA, 'factorize')
    cleanFunction.append(explore.getRuleFunction(
        CLEANDATA_REFACTOR_DATA, name, 'factorize'))
...
def testIDField(project, explore, df, dfCopy, name, stats):
    ...
    if stats['unique'] >= stats['count'] * .90:
        ...
        explore.addExploreRules(
            rules, name, CLEANDATA_DROP_ID_FIELD, value=name, key=None)

        # explore.getRuleFunction(functionName, columnName, value ,
        #                           df=None, forPredict=True):
        cleanFunction.append(explore.getRuleFunction(
            CLEANDATA_DROP_ID_FIELD, name, value=name, df=df, forPredict=True))
        dropped = True
    return rules, dropped, cleanFunction
...
def testRebucket(project, explore, df, dfCopy, name, stats):
    ...
    if stats['unique'] <= project.uniqueThreshold:
        ...
        if len(other) > 1:
            explore.addExploreRules(
                rules, name, CLEANDATA_REBUCKET, value=other, key=str(items))
            cleanFunction.append(explore.getRuleFunction(
                CLEANDATA_REBUCKET, name, value=[other, 'Others'],
                df=df, forPredict=True))

            # self.addRecommendation(name,CLEANDATA_CONVERT_CATEGORY_TO_INTEGER)
            explore.addExploreRules(
                rules, name, CLEANDATA_CONVERT_CATEGORY_TO_INTEGER)
            cleanFunction.append(explore.getRuleFunction(
                CLEANDATA_CONVERT_CATEGORY_TO_INTEGER, name))
...
def testCatgory(project, explore, df, dfCopy, name, stats):
    ...
    if stats['unique'] <= stats['count'] * .50:
        ...
        #
        # addRecommendation ( name, explore, value=None, key=None):
        # explore.getRuleFunction(functionName, ColName, value ,
        # df=None, forPredict=True):
        explore.addExploreRules(
            rules, name, CLEANDATA_CONVERT_CATEGORY_TO_INTEGER)
        cleanFunction.append(explore.getRuleFunction(
...
def testZeroFill(project, explore, df, dfCopy, name, stats):
    ...
    # Do the tests
    if stats['nullCount'] > 0:
        if stats['binary'] is True:
            explore.addExploreRules(
                rules, name, CLEANDATA_ZERO_FILL, value=None, key=None)
            cleanFunction.append(explore.getRuleFunction(
                CLEANDATA_ZERO_FILL, name, value=None, df=df, forPredict=True))

        else:
            explore.addExploreRules(
                rules, name, CLEANDATA_ZERO_FILL, value='mean', key=None)
            cleanFunction.append(explore.getRuleFunction(
                CLEANDATA_ZERO_FILL, name, value='mean',
                df=df, forPredict=True))
    return rules, dropped, cleanFunction
...
def testImbalanced(project, explore, df, dfCopy, name, stats):
    ...
    if checkForImbalanced(name, stats, df[name], isObject=False):
        explore.addExploreRules(rules, name, CLEANDATA_IMBALANCED_DATA,
                                value='Top value {} with {}%'.
                                format(stats['count']/stats['freq'],
                                       stats['top']*100.), key=None)
        cleanFunction.append(explore.getRuleFunction(
            CLEANDATA_IMBALANCED_DATA, name, value=None,
            df=df, forPredict=True))
...
def testSkewed(project, explore, df, dfCopy, name, stats):
    ...
    if skew > project.skewFactor:
        ...
        #
        # addRecommendation ( name, explore, value=None, key=None):
        # explore.getRuleFunction(functionName, ColName, value , df=None,
        # forPredict=True):
        explore.addExploreRules(
            rules, name, CLEANDATA_REFACTOR_DATA, value='skewed', key=None)
        cleanFunction.append(explore.getRuleFunction(
            CLEANDATA_REFACTOR_DATA, name, value='skewed',
            df=df, forPredict=True))
    return rules, dropped, cleanFunction
...
def testForSparse(project, explore, df, dfCopy, name, stats):
    ...
    if sparseScore > .5:
        ...
        #
        explore.addExploreRules(
            rules, name, CLEANDATA_REFACTOR_DATA, value='sparse', key=None)
        cleanFunction.append(explore.getRuleFunction(
            CLEANDATA_REFACTOR_DATA, name, value='sparse',
            df=df, forPredict=True))
        dropped = True
    return rules, dropped, cleanFunction


columnTests = {'object': [testOneHotEncoding, testRebucket,
                          testCatgory, testIDField],
               'number': [testIDField, testZeroFill,
                          testImbalanced, testSkewed],
               'bool': [testIDField],
               'date': [testIDField]
               }
...
def TEMPLATE(project, explore, df, dfCopy, name, stats):
    ...
    if TEST:
        ...
        # explore.getRuleFunction(functionName, ColName, value , df=None,
        #                         forPredict=True):
        explore.addExploreRules(rules, name , CLEANDATA, value=None,
                                key=None)
        cleanFunction.append(explore.getRuleFunction(CLEANDATA, name,
                                value=None , df=df, forPredict=True))
        dropped = True
    return rules, dropped, cleanFunction
'''
...
class exploreData(object):
    ...
    def __init__(self, df, project, name):
        ...
        self.heatMap = pd.DataFrame(
            d, index=exploreHeatMap, columns=df.columns)
    ...
    '''
    Feature Evalution does a basic conversation of string data
    for a brute-force test
    '''

    def featureEvaluation(self, project, name, df, toWeb=False):
        mlUtility.runLog(
          'Basic Automethod. Evaluating correlations and feature importance\n')
        ...
        print()
        if self.target is not None:
            self.evaluateColumnImportance(
                evaluationColumns, self.correlations,
                self.featureImportance, doDrop=True)
        return
#        try:
...
#            raise e
    ...
    '''
    Feature testing does stuff
    '''

    def featureTesting(self, project, name, df):
        mlUtility.runLog(
            'Advanced. Evaluating correlations and feature importance\n')
        bestScores = {}
        bestRules = {}
        ...
        for colName in dfCopy:
            ...
            rules, dropped = runExploreTest(
                testDefaultFactorize, self.project, self, df, dfCopy,
        ...
        featureImportance = self.calcFeatureImportance(
            dfCopy, dfCopy[self.target])
        for colName in dfCopy:
            if colName == self.target or colName == project.isTrainingSet:
                pass
            else:
                bestScores[colName] = featureImportance[colName] + \
                    abs(correlations[self.target][colName])
        ...
        for colName in dfCopy:
            ...
            if colName != self.target:
                ...
                for functionName in tests:
                    rules, dropped = runExploreTest(
                        functionName, self.project, self,
                        df, dfCopy, colName, stats)
                    ...
                    if rules is not None:
                        ...
                        featureImportance = self.calcFeatureImportance(
                            dfCopy, dfCopy[self.target])
                        encodedColumns = getOneHotEncodeColumns(
                            colName, dfCopy.columns.tolist())
                        if len(encodedColumns) > 0:
                            score = 0
                            for x in encodedColumns:
                                allRules.append(
                                    tuple((x, featureImportance[x] +
                                          abs(correlations[self.target][x]),
                                          rules)))
                                score += featureImportance[x] + \
                                    abs(correlations[self.target][x])
                            dfCopy.drop(encodedColumns, axis=1, inplace=True)
                            df.drop(encodedColumns, axis=1, inplace=True)

                        else:
                            score = featureImportance[colName] + \
                                abs(correlations[self.target][colName])
            ...
            if len(value) > 0:
                for k, v in value.items():
                    ...
                    if v is None:
                        ...
                        column_recommendations['list'].append(
                            '{} value={}'.format(k, v))
    ...
    def getRuleFunction(self, explore, name, value=None,
                        df=None, forPredict=True):
        if self.project.recommendOnly:
            return None
        else:
    ...
    def statsSummary(self, name):
        if name in self.columns:
            ...
            str = 'Stats for Column: {}  datatype={}\n'.format(
                name, dStats['dataType'])
            str += '   mean:   {:<12.4}   first:  {}\n'.format(
                dStats['mean'], dStats['first'])
            str += '   median: {:<12}   last:   {}\n'.format(
                dStats['median'], dStats['last'])
            str += '   std:    {:<12.4}   top:    {}\n'.format(
                dStats['std'], dStats['top'])
            str += '   min:    {:<12}   freq:   {}\n'.format(
                dStats['min'], dStats['freq'])
            str += '   max:    {:<12}   uniqie: {:<12}\n'.format(
                dStats['max'], dStats['unique'])
            str += ' count:    {:<12}   nulls:  {:<12}\n\n'.format(
                dStats['count'], dStats['nullCount'])
            #  str += '    1%:    {:<10}   2.5%:   {:<10}   5%:   {:<10}
            #        25%:   {:<10}    50%:   {:<10}\n'.format(stats['1%'],
            #        stats['2.5%'], stats['5%'] , stats['25%'] ,stats['50%'])
            #  str += '   50%:    {:<10}   75%:    {:<10}  95%:   {:<10}
            #         97.5%:   {:<10}    99%:   {:<10}\n'.format(stats['50%'],
            #          stats['75%'], stats['95%'],stats['97.5%'],stats['99%'])
            ...
            if stats['dataType'] == 'bool':
                ...
                fmt = '   {:9d}% {:9.1f}% {:9d}% {:9d}% {:9d}%' +\
                      ' {:9d}% {:9d}% {:9.1f}% {:9d}%\n'
                str += fmt.format(1, 2.5, 5, 25, 50, 75, 95, 97.5, 99)
                fmt = '   {:10.2f} {:10.2f} {:10.2f} {:10.2f}' +\
                      ' {:10.2f} {:10.2f} {:10.2f} {:10.2f} {:10.2f}\n'
                str += fmt.format(
                    stats['1%'], stats['2.5%'], stats['5%'], stats['25%'],
                    stats['50%'], stats['75%'], stats['95%'],
                    stats['97.5%'], stats['99%'])
            return str
    ...
    def evaluateColumnImportance(self, columns, correlations,
                                 featureImportance, doDrop=False):

        colsToDrop = self.colsToDrop
        target = self.target

        col = []
        for name, i in sorted(featureImportance.items(),
                              key=lambda item: item[1]):
        ...
        # Top 80% of
        topFI = col[round(
            len(col) * self.project.bottomImportancePrecentToCut):]
        ...
        for name, _ in sorted(targetCorrABS.items(), key=lambda item: item[1]):
            ...
            if name != target:
                if name not in colsToDrop:
                    col.append(name)
                    c = targetCorr[name]
                    if c >= self.project.correlationThreshold\
                            or c <= -(self.project.correlationThreshold):
                        self.addRecommendation(
                            name, CLEANDATA_HIGH_CORRELATION, c)
        ...
        for name in columns:
            if name != target:
                if featureImportance[name]\
                   > self.project.featureImportanceThreshold:
                    self.addRecommendation(
                        name, CLEANDATA_HIGH_FEATURE_IMPORTANCE, c)
    ...
    def reviewObjects(self, name, stats, data, project):
        ...
        if stats['unique'] <= project.uniqueThreshold:
            for item, value in histogram.items():
                if value < project.smallSample:
                    self.addRecommendation(
                        name, CLEANDATA_REBUCKET, item, str(item))

        # chck for large samples
        elif stats['unique'] > project.highDimensionality:
            self.addRecommendation(
                name, CLEANDATA_DIMENSIONALITY_REDUCTION, stats['unique'])
            ...
            if item is not np.nan:
                ...
                if item != lower:
                    if lower in histogram:
                        # mlUtility.traceLog(('   --> add recommendations',
                        #           lower, item)
                        self.addRecommendation(name, CLEANDATA_FIX_CASE, [
                                               lower, item],
                                               lower+CLEANDATA_BREAK+str(item))
                upper = item.upper()
                if item != upper:
                    if upper in histogram:
                        self.addRecommendation(name,
                                               CLEANDATA_FIX_CASE +
                                               CLEANDATA_BREAK,
                                               [upper, item],
    ...
    def calcFeatureImportance(self, df, y):
        ...
        target = self.project.targetVariable[
                 self.project.defaultPreppedTableName]
        ...
        forest = XGBClassifier(
            random_state=self.project.randomState, n_jobs=-1)

        forest.fit(df[:self.project.trainingSetLength],
                   y[:self.project.trainingSetLength])
    ...
    def reviewNumbers(self, name, stats, series):
        ...
        if checkForImbalanced(name, stats, series, isObject=False):
            self.addRecommendation(name, CLEANDATA_IMBALANCED_DATA,
                                   value='Top value {} with {}%'.format(
                                       stats['count']/stats['freq'],
                                       stats['top']*100.), key=None)
        skew = series.skew()
    ...
    def checkforOutliers(self, name, series, stats, strong=True):
        ...
        if len(level1) > 0:
            valuesL1 = [series[key]
                        for key, value in level1.items() if value is True]
            ...
            if len(below25) > 0 and strong:
                ...
                if len(level2) > 0:
                    valuesL2 = [value for mask, value in zip(
                        np.nditer(level2), below25) if mask is True]
            ...
            if len(above75) > 0 and strong:
                ...
                if len(level2) > 0:
                    valuesL2 = [value for mask, value in zip(
                        np.nditer(level2), above75) if mask is True]
    ...
    def plotExploreHeatMap(self, toWeb=False):
        ...
        sns.heatmap(self.heatMap, annot=False,
                    cbar=False, cmap='Reds', fmt='.0f')
        plt.xticks(
            rotation=45,
    ...
    def plotSegmentation(self, x, y=None, hue=None):
        if x in self.dataFrame:
            if y is None or y in self.dataFrame:
                ...
                if hue in self.dataFrame:
                    sns.lmplot(x=x, y=y, hue=hue,
                               data=self.dataFrame, fit_reg=False)
                    plt.show()
    ...
    def plotCorrelations(self):
        ...
        # Plot heatmap of correlations
        sns.heatmap(self.correlations, annot=True, mask=mask,
                    cbar=True, cmap='Greens', fmt='.3f')
        plt.show()
    ...
    def plotFeatureImportance(self):
        ...
        col = []
        imp = []
        for c, i in sorted(self.featureImportance.items(),
                           key=lambda item: item[1]):
            col.append(c)
            imp.append(i)
