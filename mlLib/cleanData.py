...
CLEANDATA_DROP_ID_FIELD = 'Identified list of unique values. Dropped as an ID field'
...
class cleanOneHotEncode(object):
    ...
    def execute(self, df, project, cleaningRules=None):
        ...
        #print ('df===\n', df)
        #mlUtility.runLog (' DF Column before hotcode run\n {}'.format(df.columns.tolist()))
        ...
        #mlUtility.runLog (' DF Column before After run\n {}'.format(df.columns.tolist()))
        return None
        ...
        for rule in self.value:
            ...
            if string is None:
                ...
                if newValue is None or len(newValue)==0:
                    newValue = string
                column = df[self.name]
                mask = column.str.contains(string, case=False, regex=usingRegex)
                df.loc[ mask , self.name]  = newValue
                otherMask = np.logical_or(mask, otherMask)
        
        #print ('df[self.name]=',df[self.name])
        #print ('len(df[self.name])=',len(df[self.name]))
        #print ('otherMask', otherMask)
        #print ('len(otherMask)', len(otherMask))
        ...
        if rule is not None:
            ...
            if type(rule.name) is not list:
                if rule.name in df.columns:
                    logToWeb(cleaningLog,'{} Rule {} for Column {} with value {}'.format(theAction, 
                                                                        rule.ruleName, rule.name, rule.value))
                elif rule.name is not None:
                    if rule.value is None:
                        logToWeb(cleaningLog,'{} Rule {} '.format(theAction, rule.ruleName))
                    else:
                        logToWeb(cleaningLog,'{} Rule {} for {} with value {}'.format(theAction, rule.ruleName, rule.name, rule.value))
                else:
                    logToWeb(cleaningLog,'{} Rule {} '.format(theAction, rule.ruleName))
            else:
                if rule.value is None:
                    logToWeb(cleaningLog,'{} Rule {} for {}'.format(theAction, rule.ruleName,rule.name))
                else:
                    logToWeb(cleaningLog,'{} Rule {} for {} with value {}'.format(theAction, rule.ruleName, rule.name, rule.value)) 
            #sizeBefore = df.shape    
...
def cleanData(df, cleaningRules, isPredict=False):
    ...
    if cleaningRules is not None:
        return doClean(df, cleaningRules.project, cleaningRules.rules, isPredict)
    return None
...
def getRule(functionName, columnName, value=None , df=None, forPredict=True):
    if functionName == CLEANDATA_MARK_MISSING:
        ...
        cleanFunction =  cleanOneHotEncode(columnName, value, forPredict=forPredict)
        ...
        cleanFunction =  cleanRefactorData(columnName, value, forPredict=forPredict)
